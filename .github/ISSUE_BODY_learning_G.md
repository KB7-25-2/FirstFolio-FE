...
- [x] 로드맵 → 시간표 → 강좌 → 퀴즈 → 시나리오 스모크 체크리스트 통과 (Front)

## 수동 스모크 체크리스트

앱을 켠 뒤 mock 시드(예·적금 ACTIVE, 3교시 진행 중) 기준으로 확인한다.  
자동화: `npx playwright test e2e/learning.spec.js --project=chromium`

1. [x] 홈 `StudyNote`「이어서 →」→ 강좌 `learning-lesson` + `page` 쿼리로 진입
2. [x] 강좌에서 「이전 컷」/「다음 컷」이동 후, 퀴즈 수료 → 시간표에서 해당 교시 COMPLETED·다음 교시 NEXT
3. [x] 남은 LESSON 전부 수료 → 시간표에 「대단원 실전 퀴즈 시작 →」CTA·시나리오 교시 해금 → 시나리오 진입
4. [x] 시나리오 수료 → 「학습 로드맵으로」→ 해당 대단원 COMPLETED·다음 대단원 ACTIVE
5. [x] 다시 홈 이어하기 위치가 다음 학습(다음 대단원 시간표 등)으로 갱신
