...
- [ ] `MainChapterScenarioQuizView.vue` 추가 (Front)
- [ ] 시작(공문서)·페르소나 프로필·클립보드 문항·결과/평가 컴포넌트 추가 (Front)
- [ ] 시작 → 문항(미선택/선택) → 결과 확인 플로우 (Front)
- [ ] `오늘의 금융 시황` 하단 바(펼침 UI) (Front)
- [ ] `studyService` 시나리오 퀴즈·`scenario_json` mock (Front)
- [ ] `studyStore` 시나리오 응시·채점·대단원 수료 연동 (Front)
- [ ] 정답 수 기반 포인트 지급 목업 (Front)
